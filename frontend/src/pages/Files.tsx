...
export default function Files() {
    ...
    try {
      ...
      const allItems = [
        ...foldersData.map((folder: any) => ({
          ...
          // Keep the size from backend (calculated recursively)
        })),
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!fileForm.category || !fileForm.category.trim()) {
    ...
    try {
      ...
      formData.append('category', fileForm.category.trim());
      formData.append('status', fileForm.status);
      
      // Add description if provided
      if (fileForm.description && fileForm.description.trim()) {
