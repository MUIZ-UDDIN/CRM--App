...
export default function Dashboard() {
  ...
  const fetchActivities = async () => {
    try {
      ...
      const now = new Date();
      const recent = data
        .filter((activity: any) => activity.completed_at || (activity.due_date && new Date(activity.due_date) < now))
        .slice(0, 10) // Show only last 10 recent activities
        .map((activity: any) => ({
          id: activity.id,
          type: activity.type?.toLowerCase() || 'task',
          description: activity.subject || activity.description || 'Activity completed',
          time: activity.completed_at ? formatTimeAgo(activity.completed_at) : formatTimeAgo(activity.created_at),
          status: activity.status || 'completed'
        }));

      const upcoming = data
        .filter((activity: any) => activity.due_date && new Date(activity.due_date) > now && !activity.completed_at)
        ...
        .map((activity: any) => ({
          ...
          contact: activity.contact_name || 'No contact',
