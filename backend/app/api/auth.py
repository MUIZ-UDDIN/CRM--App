...
async def get_current_user_info(
    ...
    """Get current user information - return actual user data from token"""
    import logging
    logger = logging.getLogger(__name__)
    ...
    try:
        ...
        # Return actual user data from token (already validated in get_current_active_user)
        return {
            "id": current_user.get("id"),
            "email": current_user.get("email"),
            "first_name": current_user.get("first_name"),
            "last_name": current_user.get("last_name"),
            "role": current_user.get("role"),  # Use actual role from token
...
async def refresh_token(current_user: dict = Depends(get_current_active_user)):
    """Refresh access token with full user context"""
    token_data = {
