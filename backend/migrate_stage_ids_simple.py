"""
Simple migration script to convert old text-based stage_ids to UUIDs in deals table
This version uses the existing FastAPI database connection
"""
import asyncio
import sys
import os
...
async def migrate_stage_ids():
    ...
    async with async_engine.begin() as conn:
        try:
            ...
            stages_result = await conn.execute(text("""
            ...
            deals_result = await conn.execute(text("""
                SELECT id, stage_id, title, pipeline_id
                FROM deals
                WHERE is_deleted = FALSE
            """))
            
            deals = deals_result.fetchall()
            ...
            for row in deals:
                ...
                if stage_uuid:
                    ...
                    await conn.execute(text("""
                        ...
                        WHERE id = :deal_id
                    """), {"new_stage_id": stage_uuid, "deal_id": deal_id})
            ...
            # Changes are automatically committed by async_engine.begin()
